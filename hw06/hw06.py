...
class VendingMachine:
    ...
    def vend(self):
        if self.number <= 0:
            ...
            if self.currentbalance < self.price:
                difference = self.price - self.currentbalance
                return f'Please add ${difference} more funds.'
            elif self.currentbalance == self.price:
                self.currentbalance = 0
                self.number = self.number - 1
                return f'Here is your {self.product}.'
            else:
                difference = self.currentbalance - self.price
                self.currentbalance = 0
                self.number = self.number - 1
                return f'Here is your {self.product} and ${difference} change.'
    
    def add_funds(self, money):
        if self.number <= 0:
            return f'Nothing left to vend. Please restock. Here is your ${money}.'
        else:
            self.currentbalance += money
