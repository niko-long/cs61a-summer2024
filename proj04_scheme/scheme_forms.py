...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        new_expression = Pair(expressions.first.rest, expressions.rest)
        new_lambda = do_lambda_form(new_expression, env)
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        ...
        def helper(expression, value):
            if expression == nil:
                return value
            else:
                value = scheme_eval(expression.first, env)
                if is_scheme_true(value):
                    return helper(expression.rest, value)
                else:
                    return False
        return helper(expressions, expressions.first)
